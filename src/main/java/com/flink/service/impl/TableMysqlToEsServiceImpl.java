...
import static com.flink.common.util.MysqlConvertUtil.*;
...
public class TableMysqlToEsServiceImpl implements TableMysqlToEsService {
    ...
    public static void  flinkMysqlToEsTime(String sourceDDL,String goalDDL,String sourceTableName,String goalTableName,String timeField,String startTime,String endTime){
        ...
        if (StringUtils.isNotEmpty(startTime) && StringUtils.isNotEmpty(endTime)){
            format = String.format("insert into %s select * from %s where %s.%s >= '%s' and  %s.%s <= '%s';", goalTableName, sourceTableName, sourceTableName, timeField, startTime, sourceTableName, timeField, endTime);
            //同步数据
            tableEnv.executeSql(format);
        }else {
             format= String.format("insert into %s select * from %s ;", goalTableName, sourceTableName, sourceTableName);
            tableEnv.executeSql(format);
        }
    }
}
